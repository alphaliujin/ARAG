...
├── MD/                     # X2MD 输出的 Markdown + 切片 JSON
├── vector_db/              # ChromaDB 向量库（按密级分集合）
├── DocScan/                # 待检文档暂存 + 中间产物
├── dedup_results/          # 跨密级去重结果（.md / .json）
...
| GET | `/tasks/active` · `/tasks/{id}` · POST `/tasks/{id}/cancel` | 任务查询与取消 |

## ⚙️ 配置

- **`backend/.env`**：`API_KEY`（认证）、`AUTH_DISABLED`（开发免认证）。
- **`runtime_settings.json`**：相似度阈值（机密 0.6 / 受限 0.7）、切片参数、上传大小、嵌入模型等，前端「设置」页可改。
...
- 默认 `AUTH_DISABLED=True` 仅为方便内网部署，**生产环境务必设置 `API_KEY` 并启用认证**。
...
├── UI/                     # React frontend
├── DOC/                    # source docs input (by classification level)
│   ├── 0Public/ 1Restricted/ 2Confidential/
├── MD/                     # X2MD output: Markdown + chunk JSON
├── vector_db/              # ChromaDB vector store (per-level collections)
├── DocScan/                # test-doc staging + intermediates
├── dedup_results/          # cross-level dedup results (.md / .json)
...
├── start.sh / stop.sh      # one-command start / stop scripts
├── deploy/                 # Linux self-contained deploy package
...
| GET | `/tasks/active` · `/tasks/{id}` · POST `/tasks/{id}/cancel` | Task query & cancellation |

## ⚙️ Configuration

- **`backend/.env`**: `API_KEY` (auth), `AUTH_DISABLED` (dev no-auth).
- **`runtime_settings.json`**: similarity thresholds (confidential 0.6 / restricted 0.7), chunk params, upload size, embedding model — editable from the UI Settings page.
...
- `AUTH_DISABLED=True` by default is for convenient intranet deployment only — **in production always set `API_KEY` and enable authentication**.
